...
public: // --------------- tests start here ----------------
    void testConstruction() {
        ...
        REQUIRE_EQUAL(source->identifier()->toText(), String{u8"file:"} + source->path().toSafeText());
        REQUIRE_EQUAL(source->isOpen(), false);
        REQUIRE_EQUAL(source->atEnd(), false);
    }
    ...
    void testCloseWithNoRead() {
        ...
        REQUIRE_EQUAL(source->path(), String{filePath.string()});
        REQUIRE_EQUAL(source->identifier()->toText(), String{u8"file:"} + source->path().toSafeText());
        REQUIRE_EQUAL(source->isOpen(), true);
        REQUIRE_EQUAL(source->atEnd(), false);
        source->close();
        REQUIRE_EQUAL(source->name(), String{u8"file"});
        REQUIRE_EQUAL(source->path(), String{filePath.string()});
        REQUIRE_EQUAL(source->identifier()->toText(), String{u8"file:"} + source->path().toSafeText());
        REQUIRE_EQUAL(source->isOpen(), false);
        REQUIRE_EQUAL(source->atEnd(), false); // we didn't read from the source.
    }
    ...
    void requireInvalidShortFileOfSize(std::size_t size) {
        ...
        try {
            ...
            REQUIRE_EQUAL(error.category(), ErrorCategory::LimitExceeded);
        }
        REQUIRE(source->isOpen() == false);
