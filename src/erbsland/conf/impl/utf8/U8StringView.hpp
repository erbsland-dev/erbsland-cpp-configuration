...
public:
    ...
    U8StringView(String &&) = delete;
    U8StringView(std::u8string &&) = delete;
...
public:
    ...
    [[nodiscard]] auto escapedSize(EscapeMode mode) const -> std::size_t;

    /// Create an escaped version of this string.
    /// @param mode The escape mode to use for escaping.
    [[nodiscard]] auto toEscaped(EscapeMode mode) const -> String;

    /// Convert text to be safe for output or logs.
    /// @param maximumSize The maximum number of *characters* for the text.
    /// @param elideLocation Where the eliding should be placed if the text needs to be shortened.
    ...
    /// Unlike the parser's normalization, this function never fails on invalid input.
    ...
    [[nodiscard]] auto toNameNormalized() const -> String;
