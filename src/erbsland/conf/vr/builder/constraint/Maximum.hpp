...
struct Maximum : ConstraintAttribute {
    ...
    explicit Maximum(const Date &value, ConstraintOptions options = {}) : _value{value}, _options{std::move(options)} {}
    explicit Maximum(const DateTime &value, ConstraintOptions options = {}) : _value{value}, _options{std::move(options)} {}
        ...
        auto constraint = std::visit(
            [&rule](const auto &value) -> impl::ConstraintPtr {
                ...
                if constexpr (std::is_same_v<T, Integer>) {
                    requireRuleTypeForConstraint(rule, u8"maximum", {
                        vr::RuleType::Integer,
                        vr::RuleType::Text,
                        vr::RuleType::Bytes,
