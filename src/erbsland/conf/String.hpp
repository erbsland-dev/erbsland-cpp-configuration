...
public: // construction
    ...
    /// @param count Number of characters.
    /// @param c The character to repeat.
    constexpr String(std::size_t count, char c) noexcept : _string(count, static_cast<char8_t>(c)) {}
...
public: // wrap some common operators
    ...
    /// @return Reference to this String.
    auto operator+=(char8_t c) noexcept -> String& {
        _string.append(1, c);
        return *this;
    }
    ...
    /// @param c The character to append.
    /// @return Reference to this String.
    auto operator+=(char c) noexcept -> String& {
        _string.append(1, static_cast<char8_t>(c));
        return *this;
    }
    ...
    /// @return Reference to this String.
    auto operator+=(const char32_t unicodeChar) noexcept -> String& {
        impl::Char{unicodeChar}.appendTo(_string);
        return *this;
    }
...
public: // wrap common functions
    ...
    void append(const char8_t character) noexcept { _string.append(1, character); }

    /// Append a character to this String.
    /// @param character The character to append.
    void append(const char character) noexcept { _string.append(1, static_cast<char8_t>(character)); }
    ...
    void append(const char32_t unicodeChar) noexcept { impl::Char{unicodeChar}.appendTo(_string); }
    ...
    auto find(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find(s._string, pos, count);
        } else {
            return _string.find(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto find(FindStr s, size_type pos = 0) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
    ...
    template<typename FindStr>
    auto rfind(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.rfind(s._string, pos, count);
        } else {
            return _string.rfind(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto rfind(FindStr s, size_type pos = npos) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.rfind(s._string, pos);
        } else {
    ...
    template<typename FindStr>
    auto find_first_of(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_first_of(s._string, pos, count);
        } else {
            return _string.find_first_of(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto find_first_of(FindStr s, size_type pos = 0) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_first_of(s._string, pos);
        } else {
            return _string.find_first_of(s, pos);
        }
    }
    ...
    template<typename FindStr>
    auto find_first_not_of(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_first_not_of(s._string, pos, count);
        } else {
            return _string.find_first_not_of(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto find_first_not_of(FindStr s, size_type pos = 0) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_first_not_of(s._string, pos);
        } else {
            return _string.find_first_not_of(s, pos);
        }
    }
    ...
    template<typename FindStr>
    auto find_last_of(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_last_of(s._string, pos, count);
        } else {
            return _string.find_last_of(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto find_last_of(FindStr s, size_type pos = npos) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_last_of(s._string, pos);
        } else {
            return _string.find_last_of(s, pos);
        }
    }
    ...
    template<typename FindStr>
    auto find_last_not_of(FindStr s, size_type pos, size_type count) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_last_not_of(s._string, pos, count);
        } else {
            return _string.find_last_not_of(s, pos, count);
        }
    }
    /// @overload
    template<typename FindStr>
    auto find_last_not_of(FindStr s, size_type pos = npos) const -> size_type {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.find_last_not_of(s._string, pos);
        } else {
            return _string.find_last_not_of(s, pos);
        }
    }

    /// Check if the string starts with a given prefix.
    /// @note This does a binary comparison to follow the original functionality of the std-library.
    /// For a Unicode code-point-based variant, please use `startsWith`.
    /// @tparam FindStr Type of the prefix string.
    /// @param s The prefix to check.
    /// @return `true` if the string starts with the prefix, `false` otherwise.
    template<typename FindStr>
    auto starts_with(FindStr s) const noexcept -> bool {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.starts_with(s._string);
        } else {
            return _string.starts_with(s);
        }
    }

    /// Check if the string ends with a given suffix.
    /// @note This does a binary comparison to follow the original functionality of the std-library.
    /// For a Unicode code-point-based variant, please use `endsWith`.
    /// @tparam FindStr Type of the suffix string.
    /// @param s The suffix to check.
    /// @return `true` if the string ends with the suffix, `false` otherwise.
    template<typename FindStr>
    auto ends_with(FindStr s) const noexcept -> bool {
        if constexpr (std::is_same_v<FindStr, String>) {
            return _string.ends_with(s._string);
        } else {
            return _string.ends_with(s);
        }
    }

    /// Check if the string contains a given substring.
    /// @note This does a binary comparison to follow the original functionality of the std-library.
    /// To force the Unicode code-point-based variant, please use `contains` with `String`.
    /// @tparam FindStr Type of the search string.
    /// @param s The substring to search for.
    /// @return `true` if the substring is found, `false` otherwise.
    template<typename FindStr>
    requires (!std::is_same_v<std::remove_cvref_t<FindStr>, String>)
    auto contains(FindStr s) const noexcept -> bool {
        return _string.find(s) != npos;
    }
...
public: // Extensions
    ...
    /// @return `true` if the string contains valid UTF-8, `false` otherwise.
    ...
    /// @throws Error (Encoding) if the string contains invalid UTF-8 data.
    [[nodiscard]] auto characterLength() const -> std::size_t;

    /// Compare this string with another using Unicode code-points.
    /// @param other The other string to compare with.
    /// @param caseSensitivity Controls whether the comparison is case-sensitive.
    /// @return The three-way comparison result.
    /// @throws Error (Encoding) if either string contains invalid UTF-8 data.
    [[nodiscard]] auto characterCompare(
        const String &other,
        CaseSensitivity caseSensitivity = CaseSensitivity::CaseSensitive) const -> std::strong_ordering;

    /// Compare this string with another using name normalization
    ...
    [[nodiscard]] auto startsWith(
        const String &other,
        CaseSensitivity caseSensitivity = CaseSensitivity::CaseSensitive) const -> bool;
    ...
    /// @param other The other string for the comparison.
    /// @param caseSensitivity Controls case sensitivity.
    /// @return `true` if this string ends with `other`.
    /// @throws Error (Encoding) if either string contains invalid UTF-8 data.
    ...
    /// @param character The separator character to split at.
    /// @param maxSplits Optional maximum number of splits.
    /// @return The list of split segments.
    /// @throws Error (Encoding) if the string contains invalid UTF-8 data.
    [[nodiscard]] auto split(char32_t character, std::optional<std::size_t> maxSplits = {}) const -> StringList;
    ...
    /// @throws Error (Encoding) if the string contains invalid UTF-8 data.
    [[nodiscard]] auto transformed(const std::function<char32_t(char32_t)> &transformer) const -> String;
    ...
    /// Escapes characters that may disrupt display or have side effects and
    /// truncates to the given number of characters, inserting an ellipsis in
    /// the middle when necessary.
    /// @param maximumSize The maximum number of characters (default 200).
    /// @return The safe text.
    [[nodiscard]] auto toSafeText(std::size_t maximumSize = 200) const -> String;
...
struct std::hash<erbsland::conf::String> {
    auto operator()(const erbsland::conf::String &s) const noexcept -> std::size_t {
