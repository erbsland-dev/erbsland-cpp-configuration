# Copyright (c) 2023-2025 Tobias Erbsland - https://erbsland.dev
...
class WorkingSet:
    ...
    def __init__(self):
        ...
        self.dir_map: dict[str, list[str]] = defaultdict(
            list
        )  # A map with all project subdirectories.
        self.file_map: dict[str, Path] = (
            {}
        )  # A map with all header file names and their paths
        self.fwd_entry_map: dict[str, list[str]] = defaultdict(
            list
        )  # A map with all collected fwd entries.
    ...
    def write_include_header(self, target_path: Path, source_path: Path = None):
        ...
        rel_target_path = Path(
            ("../" * up_count) + "src/erbsland"
        ) / target_path.relative_to(self.src_dir)
        if self.verbose:
            print(
                f'  writing "{source_path.relative_to(self.project_dir)}" -> "{rel_target_path}"'
            )
    ...
    def collect_header_files(self):
        ...
        for path in self.src_dir.rglob("*.hpp"):
            ...
            if any(
                (exclude_dir in rel_path.parts[:-1])
            ...
            if len(rel_path.parts) >= 2:
                ...
                if header_file_name in self.file_map:
                    raise ScriptError(
                        f"Header file name conflict with `{header_file_name}`!"
                    )
    ...
    def generate_fwd_files(self):
        ...
        for header_dir, entries in self.fwd_entry_map.items():
            text = self.header.replace("{year}", str(date.today().year))
            for include in self.fwd_includes:
                text += f"{include}\n"
            text += "\n\n"
            ...
            if self.verbose:
                print(f"  writing: {path.relative_to(self.project_dir)}")
    ...
    def _create_all_path_for_base_dir(self, header_dir: str) -> Path:
        if self.create_all_base_dir:
            ...
            return Path(
                f'{self.create_all_base_dir}/all_{header_dir.replace("/", "_")}.hpp'
            )
    ...
    def generate_includes(self):
        ...
        for header_dir in sorted(self.dir_map.keys()):
            ...
            self.write_include_header(
                target_path, self._create_all_path_for_base_dir(header_dir)
            )
